...
export type EmploymentType = "internship" | "full-time" | "contract" | "freelance" | "community" | "part-time"
  ...
  return (
    <span
      className={cn(
        ...
        config.className,
        current && "shadow-lg ring-2 ring-current ring-opacity-20 animate-pulse [animation-duration:2s]",
      )}
      role="status"
...
function TimelineItemComponent({ item, isLast, className, rotationInterval = 4000 }: TimelineItemProps) {
  ...
  return (
    <div className={cn("relative flex gap-6 pb-8 not-prose", className)}>
      ...
      <div className="flex flex-col items-center">
        ...
        >
          {item.icon && (
            <div className={cn("h-2 w-2", item.current ? "text-background" : "text-muted-foreground")}>{item.icon}</div>
          )}
        </div>
      ...
      <div className="flex-1 space-y-3 pb-4">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <EmploymentBadge type={item.employmentType} current={item.current} />
          </div>
        ...
        <div className="text-sm text-muted-foreground leading-relaxed min-h-[3rem] flex items-start overflow-hidden">
          <AnimatedText text={descriptions} interval={rotationInterval} className="block w-full" />
        </div>
      </div>
    </div>
  )
...
export function Timeline({ items, className, rotationInterval = 4000 }: TimelineProps) {
  return (
